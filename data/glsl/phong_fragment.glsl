...
vec3 PhongShading(vec3 point, vec3 normal, int i, int j) {
    ...
    //vec3 ambient = 0.1f*lightColor*ambientColor;
    //vec3 diffuse = 0.6f*lightColor*diffuseColor*max(dot(lightDirection, outNormal), 0.0f);
    //vec3 specular = 0.3f*lightColor*specularColor*pow(max(dot(inEye, reflectedLight), 0.0f), shininess);
    vec3 diffuse = 0.6f*dirLights[i].color*diffuseColor*max(dot(lightDirection, outNormal), 0.0f);
    vec3 specular = 0.3f*dirLights[i].color*specularColor*pow(max(dot(inEye, reflectedLight), 0.0f), shininess);
