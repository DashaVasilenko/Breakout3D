...
float CalculateShadow(int i, int j) {
    ...
    for(int x = -1; x <= 1; ++x) {
        for(int y = -1; y <= 1; ++y) {
            ...
            shadow += currentDepth - bias < pcfDepth ? 1.0f : 0.3f;
        }
    }
    shadow /= 9.0f;
...
vec3 PhongShading(vec3 point, vec3 normal, int i, int j) {
    ...
    //vec3 ambient = 0.1f*dirLights[i].color*texture(ambientColor, fragmentData.uv).rgb;
    vec3 diffuse = 0.8f*dirLights[i].color*texture(diffuseColor, fragmentData.uv).rgb*max(dot(lightDirection, outNormal), 0.0f);
    vec3 specular = 0.3f*dirLights[i].color*texture(specularColor, fragmentData.uv).rgb*pow(max(dot(inEye, reflectedLight), 0.0f), shininess);
...
void main() { 
    ...
    if (fragmentData.normal.y == 0.0f || length(vec2(0.5f) - fragmentData.uv) > 0.5f)
        discard;
    else {
        ...
        //outColor = vec4(PhongShading(fragmentData.position, fragmentData.normal, lightDir), 1.0f);
        //outColor = vec4(PhongShading(fragmentData.position, texture(normalMap, fragmentData.uv).rgb, lightDir), 1.0f);
